...
class TweetEPIC(Bot):
    def __init__(self):
        ...
        self.parser.add_argument(
            "--now", action="store_true", help="Tweet the next image now"
        )
        self.epic = EPIC()
    ...
    def poll(self):
        ...
        for image in images:
            if (
                image["date"] not in self.state["image_queue"]
        ...
        if (
            self.state["last_post_time"] < (datetime.now() - interval)
    ...
    def do_tweet(self):
        ...
        self.log.info("Posting an image")
        ...
        self.log.info(
            "One image tweeted, %s left in queue", len(self.state["image_queue"])
        )
    ...
    def post_tweet(self, image, imagefile):
        ...
        datestring = "%s %s%s" % (
            image["date"].strftime("%H:%M on %A %B"),
            image["date"].day,
            suffix(image["date"].day),
        )
        ...
        self.post(
            ...
            images=[
                Image(
                    file=imagefile,
                    mime_type="image/png",
                    description="An image of the Earth from space.",
                )
            ],
            lat=lat,
            lon=lon,
